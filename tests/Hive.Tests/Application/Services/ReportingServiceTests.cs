...
public class ReportingServiceTests
{
    ...
    #region GetCapacityAnalysisAsync — Sprint Capacity Suggestions Tests

    [Fact]
    public async Task GetCapacityAnalysisAsync_SprintSuggestions_IncludesSprintsWithLeaveImpact()
    {
        // Future sprint with leave → should appear in suggestions
        var futureSprint = new Sprint("LP_2Q25_S1");
        futureSprint.UpdateDates(
        ...
        var capacity = new SprintCapacity(futureSprint.Id, 50, 5);

        SetupCapacityMocks(
            new List<Sprint> { futureSprint },
            new List<TeamTask>(),
            directReports,
            leaves,
            new List<SprintCapacity> { capacity });

        // Act
        var result = await _service.GetCapacityAnalysisAsync();

        // Assert
        result.SprintCapacitySuggestions.Should().HaveCount(1);
        var suggestion = result.SprintCapacitySuggestions[0];
        suggestion.SprintId.Should().Be(futureSprint.Id);
        suggestion.SprintName.Should().Be("LP_2Q25_S1");
        suggestion.TotalTeamSize.Should().Be(5);
        suggestion.PeopleOnLeave.Should().Be(1);
        suggestion.CurrentAvailableMembers.Should().Be(5);
        suggestion.SuggestedAvailableMembers.Should().BeLessThan(5);
        suggestion.LeaveDaysInSprint.Should().BeGreaterThan(0);
    }

    [Fact]
    public async Task GetCapacityAnalysisAsync_SprintSuggestions_ExcludesSprintsWithNoLeave()
    {
        // Future sprint with no leave overlap → should NOT appear in suggestions
        var futureSprint = new Sprint("LP_2Q25_S1");
        futureSprint.UpdateDates(
            DateTime.UtcNow.Date.AddDays(7),
            DateTime.UtcNow.Date.AddDays(20));
        ...
        // Leave is far in the future, outside the sprint window
        var leaves = new List<Leave>
        {
            new Leave(
                directReports[0].Id,
                ...
                DateTime.UtcNow.Date.AddDays(65))
        };
        ...
        // Assert
        result.SprintCapacitySuggestions.Should().BeEmpty(
            "no leave overlaps with the future sprint");
    }
