...
4. **Check**: Run `make check` (build with warnings as errors, typecheck, lint, all tests) and confirm it succeeds
5. **Migrations**: If `HiveDbContext` or an entity's persisted shape changed, confirm a migration was added with `make migration-add` and that `HiveDbContextModelSnapshot.cs` changed with it
6. **Docs**: `CODEBASE.md` updated for every added or removed file; `ARCHITECTURE.md` for structural changes; `BUSINESS.md` for domain-rule changes; `README.md` if commands or setup changed
